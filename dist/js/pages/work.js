...
var WorkPage = {
    render: function(container) {
        ...
        var todayBillable = 0;
        todayRecords.forEach(function(r) {
            todayBillable += (r.billableAmount || r.billable_amount || 0);
        });
    ...
    renderWorkTable: function(records, contracts) {
        ...
        records.forEach(function(record) {
            ...
            html += '<td><span style="color: var(--success-color); font-weight: 600;">¥' + (record.billableAmount || record.billable_amount || 0).toFixed(2) + '</span></td>';
    ...
    calculateBillable: function() {
        ...
        for (var i = 0; i < (data.contracts || []).length; i++) {
            if ((data.contracts)[i].id == projectId) {
        ...
        contract.prices.forEach(function(price) {
            ...
            if (priceName.indexOf('水泥块') !== -1 && priceName.indexOf('吊') !== -1) {
                totalAmount += craneBlocks * (price.unitPrice || price.unit_price || 0);
            } else if (priceName.indexOf('钢梁') !== -1 && priceName.indexOf('吊') !== -1) {
                totalAmount += craneBeams * (price.unitPrice || price.unit_price || 0);
            } else if (priceName.indexOf('水泥块') !== -1 && priceName.indexOf('运输') !== -1) {
                totalAmount += truckBlocks * (price.unitPrice || price.unit_price || 0);
            } else if (priceName.indexOf('钢梁') !== -1 && priceName.indexOf('运输') !== -1) {
                totalAmount += truckBeams * (price.unitPrice || price.unit_price || 0);
            }
        });
    ...
    viewProjectPrices: function() {
        ...
        contract.prices.forEach(function(p) {
            ...
            pricesHtml += '<span style="color: var(--primary-color); font-weight: 600;">¥' + (p.unitPrice || p.unit_price || 0).toFixed(2) + '/' + (p.unit || '次') + '</span></div>';
        });
    ...
    editWork: function(id) {
        ...
        document.getElementById('billable-amount').value = record.billableAmount || record.billable_amount || 0;
